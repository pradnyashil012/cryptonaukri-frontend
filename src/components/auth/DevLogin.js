        ...
        <main className="bg-gray-900">
            <div className="max-w-xl mx-auto rounded bg-gray-900">
                ...
                <div className="bg-gray-800 shadow-lg shadow-gray-800 rounded pt-2 pb-16 ml-3 mr-3">
                    <br/>
                <div className="text-center text-gray-300 mb-6 text-2xl font-semibold">👷 Developer Sign-In</div>
                    <form className="w-4/5 mx-auto" autocomplete="off">
                            {loginError? <div className="mb-4 bg-red-400 text-white px-2 py-2 rounded text-xs">{loginError}</div>:<></>}
                        <div className="flex items-center bg-gray-700 rounded shadow-sm mb-4">
                            ...
                            <input value={email} onChange={(e)=>{setEmail(e.target.value)}} className="text-gray-300 bg-gray-700 w-full h-12 focus:outline-none rounded" type="text"  placeholder="Email " autocomplete="do-not-autofill"/>
                        </div>
                        <div className="bg-gray-700 flex items-center rounded shadow-sm">
                            ...
                            <input value={password} onChange={(e)=>{setPassword(e.target.value)}} className="text-gray-300 bg-gray-700 w-full h-12 focus:outline-none rounded" type="password" name="passwordQuicklearn" id="quicklrnpassword" placeholder="Password" autocomplete="do-not-autofill"/>

                        </div>
                        {/* <ForgotPassword /> */}
                        <button type="button" onClick={()=>{navigate('/resetPassWord')}} className="mb-4 mt-1 text-xs text-md text-gray-500 text-center underline text-cyan-500 rounded-lg py-1 px-2 m-auto">Forgot your password ?</button>
                        
                        
                        {loading?<button type="button" className="bg-indigo-400 block mx-auto text-white text-sm rounded shadow-md px-6 py-2" disabled>Loading...</button>:<button onClick={handleSubmit} className="bg-blue-600 block mx-auto text-white text-sm rounded shadow-md px-6 py-2 w-full" >Sign in</button>}
                        <br/>
                    </form>
                    <div className="text-center">
                        <a className="text-blue-400 text-md font-semibold" href="/auth/devsignup">New User ? Signup </a>
                    </div>
                </div>
            </div>
            <br/>
