...
const EditProfile = ({token, user}) =>{

    const API = process.env.REACT_APP_API_ENDPOINT;

    const [resume, setResume] = useState("");
    const [smLoading, setSmLoading] = useState(false);
    const [resumeUpdated, setResumeUpdated] = useState(0);
    ...
    return(<>
        ...
        >
    <div className="p-4 max-w-2xl m-auto mt-12 h-full md:h-auto ">
        <br/>
        <br/>
        <br/>
        <br/>
        ...
        <div className="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
            ...
            data-modal-toggle="defaultModal"
            type="button"
            className="text-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600"
            onClick={()=>setOpenEditProfile(false)}
            >
            close
            </button>
