  ...
  return (
    <div className="h-60 flex flex-col justify-between grow shrink basis-80 py-3 px-2 border border-gray-500 rounded-lg">
        <div>
            ...
            <div className="mt-2 text-gray-400">{(props.description).substr(0, 120)}...</div>
        </div>
        <div className="mt-2 flex justify-between items-center">
            <div className="text-gray-400">
                <div><span className="font-bold text-white">{props.opn}</span>{" "}openings</div>
                <div><span className="font-bold text-white">{props.exp}</span>{" "}experience</div>
            </div>
            <div>
                <button onClick={handleApply} className="px-3 py-2 border rounded-md hover:bg-gray-800">Details</button>
            </div>
        </div>
    </div>
  )
